...
description: >-
  Gera a Especificação Técnica (TechSpec) a partir de um PRD APROVADO,
  traduzindo requisitos em arquitetura e decisões técnicas prontas para
  implementação. É a SEGUNDA fase do fluxo Bliss (PRD → TechSpec → Tasks →
  Implementação).

  Use após o `prd.md` conter `Status: APROVADO PELO USUÁRIO`, quando o usuário
  pedir o desenho técnico/arquitetura de uma feature. Explora o projeto e usa
  Context7 antes de perguntar, segue o template fixo e PARA aguardando
  aprovação. Nunca implementa código.

  Não use se o PRD não estiver aprovado nem para criar a lista de tarefas
  (use create-tasks).
---
...
- Ler `AGENTS.md` e os `SKILL.md` necessários em `.agents/skills/<nome>/`
...
- Não execute a skill `create-tasks` nem inicie a criação de tarefas
...
[Definir abordagem de monitoramento:
...
[Pesquisar as skills em `.agents/skills` que se apliquem a esta especificação e listá-las abaixo:]
