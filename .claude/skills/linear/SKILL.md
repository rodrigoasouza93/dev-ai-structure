---
name: linear
description: Manage issues, projects & team workflows in Linear. Use when the user wants to read, create or update tickets in Linear via the Linear MCP server in Claude Code.
---
...
This skill provides a structured workflow for managing issues, projects, and team workflows in Linear via the Linear MCP server connected to Claude Code.

## Prerequisites

- The Linear MCP server must be configured in Claude Code settings and accessible via OAuth.
- If MCP tools are unavailable, ask the user to add the Linear MCP server in **Claude Code → Settings → MCP Servers** using the URL `https://mcp.linear.app/mcp`, then reconnect.
...
Tools are accessed via the `mcp__claude_ai_Linear__` prefix in Claude Code.
...
- **MCP not responding:** Verify the Linear MCP server is enabled in Claude Code settings and the OAuth session is active.
